...
namespace CriSample.Spectrum
{
    ...
    public abstract class SpectrumVisualizer : MonoBehaviour
    {
        ...
        /// 初期化処理
        /// </summary>
        /// <param name="createSpectrumAnalyzer">スペクトラムアナライザの作成</param>
        /// <param name="getSpectrumDataFunc">周波数データの取得処理</param>
        public void Initialize(Action<int> createSpectrumAnalyzer, Func<int, float[]> getSpectrumDataFunc)
        {
            _getSpectrumDataFunc = getSpectrumDataFunc;
            _isInitialized = true;
            createSpectrumAnalyzer?.Invoke(FrequencyResolution);
            InitializeRenderer();
        }
